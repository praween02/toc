@extends('layouts.app')
@section('title', ' - List of Submitted Applications For 6G Appliation')
@section('content')
<style>
        ...
        <!-- Start Content-->
		    <div class="container">
		        <div class="card">
		            <div class="card-body">
		            	<div class="row">
		            		<div class="col-md-6 float-left"><h3>{{ app('request')->input('action') ? ucfirst(app('request')->input('action')) : 'Submitted' }} Application</h3></div>
		            		<div class="col-md-6">
		            		    @php
		            		    	$check_record_exist = \App\Models\SixGUser::where('is_form_submit', 1)->where('user_id', current_user_id())->count();
		            		    @endphp

		            		    @if(empty($check_record_exist) && ( ! in_array('super_admin', get_roles())) && ( ! in_array('admin_view', get_roles()))))
		            				<a href="{{ route('six_g_user') }}"><button class="btn btn-primary float-end">Apply 6G Form</button></a>
		            		    @endif
				...
				<div class="row container_stats">
		            			<div class="col-md-8">
		            				<a href="{{ route('six_g_user.index') }}?tab=submitted"><button type="button" class="lesp btn btn-success {{ $sactive }}">Submitted <span class="badge badge-light">{{ $submit_application }}</span></button></a>&nbsp;&nbsp;
		            				<a href="{{ route('six_g_user.index') }}?tab=pending"><button type="button" class="lesp btn btn-danger {{ $pactive }}">Pending <span class="badge badge-light">{{ $total_application - $submit_application }}</span></button></a>&nbsp;&nbsp;
							<a href="{{ route('six_g_user.index') }}?tab=all"><button type="button" class="lesp btn btn-primary1 {{ $all_active }}">All Application <span class="badge badge-light"> {{ $total_application }}</span></button></a>
...
@push('scripts')
    ...
    <script>
    	...
    	$(document).on('click', '.cbox', function() {
    		 let v = $(this).val();
    		 let route = "{{ route('six_g_user.index') }}" + `?app_id=${v}`;
    		 location.href = route;
    	})
    	...
    	$(document).on('click', '.assigned_app', function() 
	    	{
		    		  $("#assignedApEv").html('loading...');
	    				...
	    				$.ajax({
							            url: `{{ route('assigned.experts') }}?appid=${app_id}`,
	                        type: "GET",
    	...
    	function category(id, ctgry) {
    		...
    		$.ajax({
						            url: `{{ route('sixg.category') }}?id=${id}&val=${ctgry}`,
                        type: "GET",
                        success: function(response) {
                        	 if (response.status == "success") {
    	...
    	function get_evaluation(id) 
    		{
              ...
              $("#adminEvaluateModal").modal('show');
    					...
    					$.ajax({
						            url: `{{ route('get_evaluation_reports') }}?id=${id}`,
                        type: "GET",    
                        data: {},
                        success: function(response) {
    	...
    	function sel_activity(activity) 
    		{
    			  ...
    			  let _token = $('meta[name="csrf-token"]').attr('content');
		    		$.ajax({
						            url: `{{ route('expert_based_on_activity') }}`,
                        type: "POST",    
		...
		$("#update_assigned_info").validate({
			...
			submitHandler: function(form) {
	          ...
	          $("#sbmitbtn").attr('disabled', true).html('just a moment...');
		        ...
		        $.ajax({
				            url: "{{ route('assigned_application_expert') }}",
                            type: "POST",    
                            data: formData,
                            processData: false,
                            contentType: false,
                            success: function(response) {
	                                if (response.status == 'success') {
	                                	location.href = `{{ route('six_g_user.index') }}`;
	                                } else {

        ...
        <!-- Start Content-->
		    <div class="container">
				...
				@if(in_array('super_admin', get_roles()))
				  <h2 class="text-right"><button type="button" class="lesp btn btn-success sactive">Total Application <span class="badge badge-light">{{ $total_applications }}</span></button> </h2>
				@endif

		            	<div class="row">
		            		<div class="col-md-6 float-left"><h3>{{ __('app.list_application_forms_for_poc_in_bsnl') }}</h3></div>
		            		<div class="col-md-6">
		            		    @php
		            		    ...
		            		    @if(empty($check_record_exist) && ( ! in_array('super_admin', get_roles())))
		            				<a href="{{ route('poc_bsnl') }}"><button class="btn btn-primary float-end">Apply POC Form</button></a>
		            			@endif
		            		</div>
		            	</div>
...
@push('scripts')
    ...
    <script>
    	function application_summary(react_id) {
    		...
    		let html = '';
    		$.ajax({  
						url: "{{ route('poc_bsnl.show') }}?react_id=" + react_id,  
						type: 'GET', 
						dataType: 'json', 
						beforeSend: function() {

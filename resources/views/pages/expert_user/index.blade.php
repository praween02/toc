@extends('layouts.app')
@section('title', ' - List of Submitted Applications For Expert')
@section('content')
<style>
		        ...
		        <div class="card">
		            <div class="card-body">
		            	<div class="row">
		            		...
		            		<div class="col-md-6">
		            			<button class="btn btn-primary float-end" onclick="window.location = `{{ route('expert_user') }}`" {!! count($users) ? 'disabled="disabled"' : '' !!}>
...
<script type="text/javascript">
    ...
    <script>
    	function application_summary(react_id) {
    		...
    		let html = '';
    		$.ajax({  
						url: "{{ route('expert_user.show') }}?react_id=" + react_id,  
						type: 'GET', 
						dataType: 'json', 
						beforeSend: function() {
					  	...
					  	success: function(data) { 
					    	...
					    	if (status == 'success') {
					    		Object.keys(response).forEach(key => {
					    			if (!response[key] || key == 'id' || key == 'user_id') return;
					    			var label = key.split('_').map(item => {
					    				return item.toUpperCase();
					    			})
					    			console.log(label);
					    			var name = /\.(png|pdf|jpg|jpeg|gif|xlsx|ppt)\b/i.test(response[key]) ? `<a href="${path}${response[key]}">View</a>` : response[key];
					    			html += `<tr>
										      <td><strong>${label.join(' ')}</strong></th>
										      <td>${name}</td>
										    </tr>`;
					    		})
					    			/*html = `<tr>
